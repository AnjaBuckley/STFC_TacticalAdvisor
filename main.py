    ...
    if _HOSTILES_PATH.exists():
        ...
        for h in hostiles:
            if h["name"].lower() == name_lower:
                return h
    raise ValueError(
        "Unknown or ambiguous target. Use --list and the exact catalogue name."
    )
...
def recommend(
    task: str = typer.Option(
        "pve_hostile", help="Task type: pve_hostile, pvp, duo_wave_defense, etc."
    ),
    target: str = typer.Option(
        "Gorn Hunter", help="Target hostile name (must match hostiles.json)"
    ),
    profile: str = typer.Option(
        "profiles/player_profile.json", help="Path to player profile"
    ),
    top: int = typer.Option(3, help="Number of recommendations to show"),
    level: int = typer.Option(
        None, help="Hostile level (exact level; default: highest available)"
    ),
    hostile_id: int = typer.Option(None, help="Exact hostile variant ID"),
    list_targets: bool = typer.Option(
        False, "--list", help="List all available target names"
    ),
):
    if list_targets:
        if _HOSTILES_PATH.exists():
            ...
            for h in hostiles:
                typer.echo(
                    f"  {h['name']:35s} [{h['type']:20s}] Lvl {h['level_range'][0]}-{h['level_range'][1]}"
                )
        return
    ...
    from service import mission_target, validate_profile

    try:
        validate_profile(player_profile)
        selected = _load_hostile(target)
        hostile = mission_target(
            task, selected["name"], level, selected["ship_class"], {}, hostile_id
        )
