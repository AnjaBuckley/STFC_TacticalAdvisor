...
Keep JSON files on LF line endings through `.gitattributes`: public snapshot and mapping hashes validate exact bytes. Windows checkout conversion to CRLF would quarantine valid mechanics. Do not disable integrity checks to work around a checkout mismatch.
