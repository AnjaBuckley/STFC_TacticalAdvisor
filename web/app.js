...
function updateTargets() {
 ...
 for(const [key,label] of [['hp','Hull health'],['base_damage','Damage per round']]) {
  const input=$(`#stat-${key}`);input.required=pvp;
  $(`label[for="stat-${key}"]`).innerHTML=esc(label)+(pvp?' <span class="required-label">Required</span>':'');
 }
 $('.target-details summary span').textContent=pvp?'2 required fields':'Optional overrides';
 $('.target-details > p').textContent=pvp?'Hull health and damage per round are required. Enter shield health separately; other fields are optional. Percentages use 0–100.':'Use values from your battle log. Blank fields use the target catalogue. Percentages use 0–100.';
 $$('#target-stats input').forEach(i=>i.placeholder=pvp?(i.required?'Required':'Optional'):'Catalogue');
...
function markRequiredFields(ids) {
 for(const id of ids) {
  const input=document.getElementById(id),label=$(`label[for="${id}"]`);
  input.required=true;
  if(label && !$('.required-label',label))label.insertAdjacentHTML('beforeend',' <span class="required-label">Required</span>');
 }
}
