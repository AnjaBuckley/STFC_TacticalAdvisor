...
const statFields = [
 ['hp','Hull + shield health',1,false],['base_damage','Damage per round',0,false],
...
function updateAccountChrome() {
 ...
 $('#ship-select').innerHTML=options([['','Compare available ships'],...state.profile.ships.filter(s=>s.available!==false && s.ship_class!=='Survey').map(s=>[s.name,`${s.name} · Level ${s.level || 1}`])],selected);
}
function updateTargets() {
 ...
 const filter=h=> task==='wave_defense'?h.name==='Solo Wave Defense':task==='duo_wave_defense'?h.name==='Duo Wave Defense':task==='pve_academy_drone'?h.type==='Academy Drone':task==='galactic_anomaly'?h.type==='Galactic Anomaly':task==='dreadnought'?h.type==='Dreadnought':!['Wave Defense','Academy Drone','Galactic Anomaly','Dreadnought'].includes(h.type);
...
function updateBriefing(resetLevel=false) {
 ...
 if(pvp) {
  ...
  if(resetLevel) $('#target-level').value=Math.min(state.profile.ops_level,target.level_range?.[1] || 60);
...
async function runMission(event) {
 ...
 const body={task_type:$('#task').value,target_name:$('#target').value,level:Number($('#target-level').value),top_n:Number($('#top-n').value),ship_name:$('#ship-select').value || null,enemy_class:$('#enemy-class').value,target_stats:stats};
...
function renderResults() {
 ...
 <div class="field-help">Isolytic bonus: ${percent(sim.isolytic_bonus*100)} · Critical Mitigation: ${percent(rec.critical_mitigation.total_crit_mitigation*100)}${rec.cost_per_kill?` · Estimated repair resources / kill: ${number(rec.cost_per_kill)}`:''}</div>
 ...
 <details class="reasoning"><summary>First simulated round · incoming damage layers</summary><p class="field-help">${trace.is_critical?'Critical':'Normal'} incoming hit, first seeded run. Both damage tracks use Apex once.</p><div class="trace">${[['raw','Raw damage'],['after_standard','After standard mitigation'],['after_apex','After Apex + critical multiplier'],['after_critical','After Critical Mitigation'],['isolytic','Parallel isolytic'],['decay','Decay']].map(([k,n])=>`<div><small>${n}</small><strong>${number(trace[k])}</strong></div>`).join('')}</div></details>
 ${sim.unmodelled_abilities.length?`<details class="reasoning" open><summary>${sim.unmodelled_abilities.length} ability effects need battle-log verification</summary><p class="field-help">These effects can inform heuristic ranking but are excluded from the numeric simulation. Conditions, timing, or exact values are incomplete.</p><ul>${sim.unmodelled_abilities.map(a=>`<li>${esc(a)}</li>`).join('')}</ul></details>`:''}</div></article>
...
async function saveShip(event) {
 ...
 try {
  ...
  const ship={...selected,...old,name:$('#edit-name').value.trim(),ship_class:$('#edit-class').value,level:Number($('#edit-level').value),tier:Number($('#edit-tier').value),base_stats:{...old.base_stats},simulacrum_refit:$('#edit-refit').checked,isolytic_damage_bonus:Number($('#edit-iso').value)/100};
  for(const k of ['attack','health','armor','shield_deflection','dodge','armor_piercing','shield_piercing','accuracy'])ship.base_stats[k]=Number($(`#edit-${k}`).value);
...
async function importFile(form) {
 ...
 try {
  ...
  preview=await api(`/api/import/preview?kind=${form.dataset.importKind}`,{method:'POST',body:data});
...
document.addEventListener('change',async event=>{
 ...
 if(el.id==='catalog-ship'){
  const s=state.ships.find(s=>s.name===el.value);if(s){$('#edit-name').value=s.name;$('#edit-class').value=s.ship_class;}
 }
